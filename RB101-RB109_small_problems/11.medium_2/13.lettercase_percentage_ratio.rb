...
def letter_percentages(string)
  ...
  neither = string.chars.select { |char| char.match?(/[^a-zA-Z]/) }
  hsh = {}
  hsh[:lowercase] = ((lowercase.count.to_f / string.size) * 100).round(1)
  hsh[:uppercase] = ((uppercase.count.to_f / string.size) * 100).round(1)
  hsh[:neither] = ((neither.count.to_f / string.size) * 100).round(1)
  # hsh[:neither] =  (100 - hsh[:lowercase] - hsh[:uppercase]).round(1)
  hsh
end
...
p letter_percentages('Launch School is GREAT') == {lowercase: 54.5, uppercase: 31.8 , neither: 13.6 }
